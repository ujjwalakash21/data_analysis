...
;with cte as (
select Century, theme_name, COUNT(set_num) total_set_num,sum(COUNT(set_num)) OVER() as total
	from analytics_main
	where Century = '21st_Century'
	group by Century, theme_name
	)

	SELECT Century,theme_name,total_set_num, total, ROUND((1.00*total_set_num/total)*100,2) AS percentage
	FROM cte
